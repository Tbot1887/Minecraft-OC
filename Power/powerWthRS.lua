--[[ this based in code from jennifer cally "ebony"
See Original Here: https://github.com/ebonyfaye/Minecraft-OC/blob/master/Power/Power.lua

Whats Different?
Added Support for Mekanism Energy Cubes
Added ability to send redstone signal to a nuclearcraft reactor
--]]
...
-- Needed for redstone Control of reactor
local sides = require( "sides" )
rsOut = component.redstone
...
function getCells()
	...
	local MekCell = component.list("elite_energy_cube")
	...
	for address, name in pairs(MekCell) do
		countMekCell = countMekCell+1
		
		if countMekCell > 1 then
			cellsID[address] = "Mekanism Energy Cube".." "..
			countMekCell
		else
			cellsID[address] = "Mekanism Energy Cube"
		end
...
while true do
  ...
  --Check power level and emit redstone signal if greater then 25%
  if(totalPower < (totalMaxPower*0.25) then
    rsOut.setOutput(sides.front, 15)
  else
	rsOut.setOutput(sides.front, 0)
  end
